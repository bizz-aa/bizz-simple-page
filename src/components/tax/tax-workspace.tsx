...
export function DetailsDrawer({
  open, onClose, title, description, rows, footer,
}: {
  open: boolean;
  onClose: () => void;
  title: string;
  ...
  return (
    <Sheet open={open} onOpenChange={(next) => !next && onClose()}>
      <SheetContent className="w-full border-white/10 bg-[#0f1115] text-white sm:max-w-md">
        <SheetHeader>
        ...
        <dl className="mt-6 space-y-1">
          {rows.map((row) => (
            <div key={row.label} className="flex items-start justify-between gap-4 rounded-xl px-3 py-2.5 odd:bg-white/[0.04]">
              <dt className="text-xs uppercase tracking-[0.14em] text-white/45">{row.label}</dt>

...
import {
  ...
  FileBarChart, BarChart3, Award, ClipboardList,
} from "lucide-react";
import { toast } from "sonner";
import sunsetBg from "@/assets/sunset-bg.jpg";

export const Route = createFileRoute("/_authenticated/m/employees")({ component: EmployeesHub });

function EmployeesHub() {
  const navigate = useNavigate();

  const cards = [
    { label: "Employees", icon: Users, onClick: () => toast.info("Employees — coming soon") },
    { label: "Departments", icon: Building2, onClick: () => toast.info("Departments — coming soon") },
    { label: "Attendance", icon: Clock, onClick: () => toast.info("Attendance — coming soon") },
    { label: "Payroll", icon: Banknote, onClick: () => toast.info("Payroll — coming soon") },
  ];
  ...
  const hrItems = [
    ...
    { label: "Payslips", icon: FileBarChart },
  ];

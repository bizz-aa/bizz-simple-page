...
import {
  ...
  Home, Scan, Package, MoreHorizontal, ChevronRight, BarChart3,
} from "lucide-react";
...
function SalesHub() {
  ...
  const cards = [
    { label: "New Sale", icon: ShoppingCart, onClick: () => navigate({ to: "/pos" }) },
    { label: "Invoices", icon: Receipt, onClick: () => toast.info("Invoices — coming soon") },
    { label: "Quotations", icon: FileText, onClick: () => toast.info("Quotations — coming soon") },
    { label: "Payments", icon: Wallet, onClick: () => toast.info("Payments — coming soon") },
  ];

  const moreItems = [
    { label: "Sales Orders", icon: ClipboardList, onClick: () => toast.info("Sales Orders — coming soon") },
    { label: "Sales History", icon: History, onClick: () => toast.info("Sales History — coming soon") },
    { label: "Returns", icon: RotateCcw, onClick: () => toast.info("Returns — coming soon") },
    { label: "Draft Orders", icon: FileText, onClick: () => toast.info("Drafts — coming soon") },
    { label: "Receipts", icon: Receipt, onClick: () => toast.info("Receipts — coming soon") },
  ];
    ...
    >
      ...
      <nav className="fixed inset-x-0 bottom-0 z-30 border-t border-white/20 bg-black/40 backdrop-blur-xl lg:hidden">
        <div className="mx-auto flex max-w-md items-center justify-around px-4 py-3">
          ...
          <BottomBtn label="Sales" icon={ShoppingCart} active onClick={() => navigate({ to: "/m/sales" })} />
